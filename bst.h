...
class BinarySearchTree {
	...
	//      element in the left subtree is strictly less that the datum
	//      in the node
	//      -- AND --
	//      the node's right subtree obeys the sorting invariant, and every
	//      element in the right subtree is greater than or equal to the
	//      datum in the node
	// Again, "less than" and "greater than or equal to" are as defined
	// by the Compare functor. Note that "greater than or equal to" and "greater
	// than" end up meaning the same thing when duplicates are not allowed.
...
private:
	...
	static int height_impl(const Node* node) {
		int height(0);
		if (empty_impl(node) != true) {
			height = 1 + std::max(height_impl(node->left), height_impl(node->right));
		}
		return height;
	}
	...
	static Node* copy_nodes_impl(Node* node) {
		if (node == NULL) // if empty
			return NULL;
		else {	//If tree contains nodes
			Node* temp = new Node; //create new node
			temp->datum = node->datum;// and copy data
			temp->left = copy_nodes_impl(node->left);	//do it for the left side of the node
			temp->right = copy_nodes_impl(node->right); // for the right side of the node
			return temp;
		}
	}
	...
	static void destroy_nodes_impl(Node* node) {
		if (node == NULL) //if empty
			...
			destroy_nodes_impl(node->left);	//go to the left side of the node
			destroy_nodes_impl(node->right);	// right side
			delete node;	// delete the node at last (after traversing)
		}
	...
	static Node* find_impl(Node* node, const T& query, Compare less) {
		if (node == NULL || (!less(node->datum, query)
			&& !less(query, node->datum)))
			return node;
	...
	static Node* insert_impl(Node* node, const T& item, Compare less) {
		if (node == NULL) {
			node = new Node(item, NULL, NULL);//single node
			return node;
		}
		else if (less(node->datum, item)) {	// compare datum to decide which side of the node to insert
	...
	static Node* min_element_impl(Node* node) {
		if (node->left == NULL) // If there's no left, current node is minimum
			return node;
		else
			return min_element_impl(node->left);	// minimum element is located at leftest side of the node.
	}
	...
	static Node* max_element_impl(Node* node) {
		if (node->right == NULL)
			...
			return max_element_impl(node->right);// maximum element is located at leftest side of the node.
	}
	...
	static bool check_sorting_invariant_impl(const Node* node, Compare less) {
		...
		if (node->left != NULL && less(node->datum, node->left->datum))
			return false;


		if (node->right != NULL && less(node->right->datum, node->datum))
			return false;
	...
	static Node* min_greater_than_impl(Node* node, const T& val, Compare less) {
		...
		if (node->left == NULL
			...
			return NULL;
		//For leaf nodes
		if ((less(val, node->datum) && node->left == NULL)
			|| (node->left != NULL && less(val, node->datum)
